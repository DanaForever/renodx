// ---- Created with 3Dmigoto v1.3.16 on Mon Mar 09 02:39:53 2026
  ...
  out float4 o0 : SV_Target0)
{
  ...
  float4 uv = float4(w0.x, w0.y, w0.x, w0.y);
  r0.xyzw = uv * cb0[67].zwzw + cb0[67].xyxy;
  // r0.xyzw = w0.xyzw * cb0[67].zwzw + cb0[67].xyxy;
  ...
  // r0.xyz = max(float3(0.00499999989,0.00499999989,0.00499999989), r0.xyz);
  // r0.xyz = min(float3(2000,2000,2000), r0.xyz);
  // r0.xyz = float3(-0.00499999989,-0.00499999989,-0.00499999989) + r0.xyz;
  // r0.xyz = float3(0.000500001246,0.000500001246,0.000500001246) * r0.xyz;
  // r1.x = dot(float3(1.73125386,-0.604043067,-0.0801077113), r0.xyz);
  // r1.y = dot(float3(-0.131618932,1.13484156,-0.00867943279), r0.xyz);
  // r1.z = dot(float3(-0.0245682523,-0.125750408,1.06563699), r0.xyz);
  // r0.xyz = float3(25,25,25) * r1.xyz;
  r0.rgb = renodx::color::pq::DecodeSafe(r0.rgb, RENODX_DIFFUSE_WHITE_NITS);
  //   r1.x = dot(float3(1.73125386,-0.604043067,-0.0801077113), r0.xyz);
  //   r1.y = dot(float3(-0.131618932,1.13484156,-0.00867943279), r0.xyz);
  //   r1.z = dot(float3(-0.0245682523,-0.125750408,1.06563699), r0.xyz);
  //   r0.xyz = float3(12.5,12.5,12.5) * r1.xyz;
  r0.rgb = renodx::color::bt709::from::AP1(r0.rgb);
  r0.rgb *= RENODX_DIFFUSE_WHITE_NITS / 80.f;
  if (cb0[65].x != 0) {
    r1.xyz = log2(r0.xyz);
