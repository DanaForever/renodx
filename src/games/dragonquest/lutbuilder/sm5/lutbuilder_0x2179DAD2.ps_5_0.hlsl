...
MobileTonemapResult mobile_tonemap(float3 bt709_input, ContrastConfig contrast_config) {
  ...
  // ContrastConfig contrast_config = CreateContrast(
  ...
  r0.rgb = unrealengine::filmtonemap::ApplyMobileTonemap(r0.rgb, contrast_config);
...
float3 postprocessing_hdr(float3 bt709_input, float3 sdr) {
  ...
  // float hdr_y = renodx::color::y::from::BT709(bt709_input);
  // float sdr_y = renodx::color::y::from::BT709(sdr);
  // float scale = renodx::math::SafeDivision(hdr_y, sdr_y, 1.f);
  ...
  out float4 o0 : SV_Target0)
{
  ...
  MobileTonemapResult result = mobile_tonemap(r1.rgb, contrast_config);
  ...
  if (RENODX_TONE_MAP_TYPE == 0.f || RENODX_TONE_MAP_TYPE == 2.f) {
    ...
    if (RENODX_TONE_MAP_TYPE == 2.f) {
      ...
      hdr1 = unrealengine::filmtonemap::extended::ApplyContrastExtended(hdr1, contrast_config);
    ...
    if (RENODX_TONE_MAP_TYPE == 1.f) {
      ...
      graded_color = unrealengine::filmtonemap::extended::ApplyContrastExtended(hdr1, contrast_config);
    }
  ...
  if (cb0[56].z == 0) {
    ...
    if (r0.w != 0) {
      ...
      if (r0.w != 0) {
        ...
        r1.w = max(r7.y, r7.z);
        r1.w = max(r1.w, r7.w);
        r6.xy = max(float2(1.00000001e-010,0.00999999978), r1.ww);
        ...
        r0.w = 3.32192802 * r1.w;
        r6.z = exp2(r0.w);
        r6.xyz = float3(-3.50738446e-005,-3.50738446e-005,-3.50738446e-005) + r6.xyz;
        r7.x = dot(r4.xyz, r6.xyz);
        r7.y = dot(r5.xyz, r6.xyz);
        r7.z = dot(r3.xyz, r6.xyz);
        r6.xyz = float3(9.99999975e-005,9.99999975e-005,9.99999975e-005) * r7.xyz;
        r6.xyz = log2(r6.xyz);
