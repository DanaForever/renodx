...
float3 postprocessing_sdr(float3 bt709_input) {
  ...
  // r3.xyz = exp2(r2.xyz);
  r3.rgb = renodx::math::SafePow(r2.rgb, cb0[18].y);

  return r3.rgb;
}

float3 postprocessing_hdr(float3 bt709_input) {
...
void main(
  ...
  if (RENODX_TONE_MAP_TYPE == 0.f || RENODX_TONE_MAP_TYPE == 2.f) {
    ...
    if (RENODX_TONE_MAP_TYPE == 2.f) {
      ...
      o0.rgb = postprocessing_hdr(hdr1);
    } else {
      sdr1 = postprocessing_sdr(sdr1);
      o0.rgb = sdr1;
    }
    ...
    if (RENODX_TONE_MAP_TYPE == 1.f) {
      ...
      graded_color = postprocessing_sdr(graded_color);
    }
  ...
  if (asuint(cb0[40].w) == 0) {
    ...
    r2.rgb = renodx::color::srgb::EncodeSafe(r3.rgb);
  } else {
