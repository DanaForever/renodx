...
float3 ToneMapLMS(float3 untonemapped, float paperWhite, float peakWhite) {
  float3 color;
  color = LMS_Vibrancy(untonemapped, shader_injection.tone_map_lms_vibrancy, shader_injection.tone_map_lms_contrast);
  
  float peak = peakWhite / paperWhite;

  float3 dechroma = CastleDechroma_CVVDPStyle_NakaRushton(color);
  color = lerp(color, dechroma, shader_injection.tone_map_lms_dechroma);
