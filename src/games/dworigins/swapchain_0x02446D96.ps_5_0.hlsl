...
void frag_main()
{
  ...
  SV_Target.rgb = renodx::color::pq::Encode(color, 203.f);
    ...
    // o0.rgb = color;
    // o0.w = r0.w;
  // float _259 = exp2(log2(max(dp3_f32(float3(0.627399981021881103515625f, 0.329299986362457275390625f, 0.0432999990880489349365234375f), _239) * 3.0f, 0.0f) * 0.00999999977648258209228515625f) * 0.1593017578125f);
  // float _260 = exp2(log2(max(dp3_f32(float3(0.069099999964237213134765625f, 0.91949999332427978515625f, 0.011400000192224979400634765625f), _239) * 3.0f, 0.0f) * 0.00999999977648258209228515625f) * 0.1593017578125f);
  // float _261 = exp2(log2(max(dp3_f32(float3(0.01640000008046627044677734375f, 0.087999999523162841796875f, 0.895600020885467529296875f), _239) * 3.0f, 0.0f) * 0.00999999977648258209228515625f) * 0.1593017578125f);
  // SV_Target.x = exp2(log2(mad(_259, 18.8515625f, 0.8359375f) / mad(_259, 18.6875f, 1.0f)) * 78.84375f);
  // SV_Target.y = exp2(log2(mad(_260, 18.8515625f, 0.8359375f) / mad(_260, 18.6875f, 1.0f)) * 78.84375f);
  // SV_Target.z = exp2(log2(mad(_261, 18.8515625f, 0.8359375f) / mad(_261, 18.6875f, 1.0f)) * 78.84375f);
