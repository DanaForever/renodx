// ---- Created with 3Dmigoto v1.3.16 on Sun Jun 22 01:00:58 2025
#include "common.hlsl"
#include "shared.h"
  ...
  out float4 o0 : SV_Target0)
{
  ...
  if (RENODX_TONE_MAP_TYPE != 0.f && shader_injection.tone_map_mode == 0.f) {
    ...
    if (shader_injection.colorGradeLUTSampling == 1.f)
      lut_color = renodx::lut::SampleTetrahedral(t1, encoded, cb0[128].z + 1u);
    else {
      float3 precompute = cb0[128].xyz;
      lut_color = renodx::lut::Sample(t1, s0_s, encoded, precompute);
    }
  ...
  if (shader_injection.colorGradeLUTSampling == 0.f) {
    ...
    o0.xyz = renodx::lut::SampleTetrahedral(t1, r0.xyz, cb0[128].z + 1u);
