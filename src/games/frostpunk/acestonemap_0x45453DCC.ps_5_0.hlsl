...
// unknown dcl_: dcl_resource_texture1d (float,float,float,float) t4
  ...
  r0.w = r0.w * r0.w;
  r0.xyz = r0.xyz * r0.www;
  r0.w = saturate(renodx::color::y::from::BT709(r0.xyz));
  o0.xyz = r0.xyz;
  ...
  if (RENODX_TONE_MAP_TYPE >= 3.f) {
    ...
    o0.rgb = renodx::tonemap::psycho::psychotm_test11(
        ...
        RENODX_TONE_MAP_CONTRAST / RENODX_TONE_MAP_SATURATION,  // Offset what Naka Rushton will do to contrast
        1.0,                                                    // Macleod Boytnon-Purity (old saturation)
        1.f - RENODX_TONE_MAP_BLOWOUT,
        100.f,
        0.5f,                       // hue_restore
        1.f,                        // adaptation_contrast (unbounded Naka Rushton)
        1,                          // 0 = Neutwo, 1 = Naka Rushton
        RENODX_TONE_MAP_SATURATION  // cone_response_exponent (bounded Naka Rushton)
    );
  }
