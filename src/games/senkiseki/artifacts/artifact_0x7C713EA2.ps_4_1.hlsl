// ---- Created with 3Dmigoto v1.3.16 on Thu Jul 03 02:30:32 2025
#include "../shared.h"
cbuffer _Globals : register(b0)
{
  ...
  float BloomIntensity : packoffset(c84.z) = {1};
  float MaskEps : packoffset(c84.w);
  float4 PointLightParams : packoffset(c85) = {0,2,1,1};
  float4 PointLightColor : packoffset(c86) = {1,0,0,0};
}
  ...
  out float4 o2 : SV_TARGET2)
{
  ...
  // r3.x = saturate(dot(r1.yzw, r3.xyz));
  r3.x = (dot(r1.yzw, r3.xyz));
  r1.x = Shininess * r1.x;
  r3.y = dot(Light0.m_direction.xyz, r1.yzw);
  r3.y = r3.y * 0.5 + 0.5;
  r3.y = r3.y * r3.y;
  r2.xyz = r2.xyz * r2.www + Light0.m_direction.xyz;
  r2.w = dot(r2.xyz, r2.xyz);
  r2.w = rsqrt(r2.w);
  r2.xyz = r2.xyz * r2.www;
  // r2.x = saturate(dot(r1.yzw, r2.xyz));
  r2.x = (dot(r1.yzw, r2.xyz));
  // r2.x = log2(r2.x);
  // r2.x = SpecularPower * r2.x;
  // r2.x = exp2(r2.x);
  r2.x = renodx::math::SafePow(r2.x, SpecularPower);
  r2.x = min(1, r2.x);
  r1.x = r2.x * r1.x;
  r2.xyz = Light0.m_colorIntensity.xyz * r3.yyy + scene.GlobalAmbientColor.xyz;
  r2.xyz = min(float3(1.5,1.5,1.5), r2.xyz);
  r2.xyz = Light0.m_colorIntensity.xyz * r1.xxx + r2.xyz;
  r0.w = 1 + -r0.w;
  r3.yzw = r2.xyz * scene.MiscParameters1.xyz + -r2.xyz;
  r2.xyz = r0.www * r3.yzw + r2.xyz;
  ...
  r1.x = renodx::color::y::from::BT709(r2.xyz);
