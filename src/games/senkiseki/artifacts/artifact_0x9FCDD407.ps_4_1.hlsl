...
cbuffer _Globals : register(b0)
{
  ...
  float CubeMapIntensity : packoffset(c85.y) = {1};
  float CubeMapFresnel : packoffset(c85.z) = {0};
  float BloomIntensity : packoffset(c85.w) = {1};
  float GlareIntensity : packoffset(c86) = {1};
  float MaskEps : packoffset(c86.y);
...
SamplerState EmissionMapSamplerSampler_s : register(s7);
SamplerState GlareMapSamplerSampler_s : register(s8);
...
Texture2D<float4> EmissionMapSampler : register(t5);
TextureCube<float4> CubeMapSampler : register(t6);
  ...
  out float4 o2 : SV_TARGET2)
{
  ...
  r0.w = EmissionMapSampler.Sample(EmissionMapSamplerSampler_s, v3.xy).x;
