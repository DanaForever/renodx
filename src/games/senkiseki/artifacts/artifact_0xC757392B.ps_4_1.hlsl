// ---- Created with 3Dmigoto v1.3.16 on Thu Jul 10 22:53:56 2025
#include "../shared.h"
...
cbuffer _Globals : register(b0)
{
  ...
  float CubeMapIntensity : packoffset(c87.w) = {1};
  float CubeMapFresnel : packoffset(c88) = {0};
  float BloomIntensity : packoffset(c88.y) = {1};
  float MaskEps : packoffset(c88.z);
...
SamplerState PointWrapSamplerState_s : register(s2);
SamplerState PointClampSamplerState_s : register(s3);
SamplerState DiffuseMapSamplerSampler_s : register(s5);
SamplerState NormalMapSamplerSampler_s : register(s6);
SamplerState SpecularMapSamplerSampler_s : register(s7);
SamplerState DiffuseMap2SamplerSampler_s : register(s8);
SamplerComparisonState LinearClampCmpSamplerState_s : register(s4);
...
TextureCube<float4> CubeMapSampler : register(t7);
  ...
  out float4 o2 : SV_TARGET2)
{
      ...
      if (r3.w != 0) {
        ...
        if (r3.w != 0) {
          ...
          if (r2.y != 0) {
            ...
            r2.z = LightShadowMap0.SampleCmpLevelZero(LinearClampCmpSamplerState_s, r7.xy, r7.z).x;
  ...
  r3.xyz = NormalMapSampler.Sample(NormalMapSamplerSampler_s, v3.xy).xyz;
  ...
  r4.w = r4.w * 0.5 + 0.5;
  r4.w = r4.w * r4.w;
  r5.xyz = SpecularColor.xyz * Light0.m_colorIntensity.xyz;
  r4.xyz = r4.xyz * r2.www + scene.FakeRimLightDir.xyz;
  r2.w = dot(r4.xyz, r4.xyz);
  r2.w = rsqrt(r2.w);
  ...
  r5.xyz = RimLitColor.xyz * r1.www;
  r4.xyz = r5.xyz * r2.xyz + r4.xyz;
  r4.xyz = min(RimLightClampFactor, r4.xyz);
