...
cbuffer _Globals : register(b0)
{
  ...
  float PerMaterialMainLightClampFactor : packoffset(c64) = {1.5};
  float3 LightDirForChar : packoffset(c64.y);
  float GameMaterialID : packoffset(c65) = {0};
  float4 GameMaterialDiffuse : packoffset(c66) = {1,1,1,1};
  float4 GameMaterialEmission : packoffset(c67) = {0,0,0,0};
  float GameMaterialMonotone : packoffset(c68) = {0};
  float4 GameMaterialTexcoord : packoffset(c69) = {0,0,1,1};
  ...
  float SphereMapIntensity : packoffset(c77.w) = {1};
...
Texture2D<float4> DiffuseMap3Sampler : register(t3);
Texture2D<float4> CartoonMapSampler : register(t4);
Texture2D<float4> SphereMapSampler : register(t5);
  ...
  out float4 o2 : SV_TARGET2)
{
  ...
  o1.w = 0.5 * PerMaterialMainLightClampFactor;
  ...
  r3.xyz = min(PerMaterialMainLightClampFactor, r3.xyz);
  ...
  r0.w = renodx::color::y::from::BT709(r1.xyz);
