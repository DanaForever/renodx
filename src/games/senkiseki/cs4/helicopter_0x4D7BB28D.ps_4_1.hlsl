// ---- Created with 3Dmigoto v1.3.16 on Thu Jun 26 14:13:30 2025
#include "../shared.h"
cbuffer _Globals : register(b0)
{
  ...
  float3 ShadowColorShift : packoffset(c83) = {0.100000001,0.0199999996,0.0199999996};
  float Shininess : packoffset(c83.w) = {0.5};
  float SpecularPower : packoffset(c84) = {50};
  float3 RimLitColor : packoffset(c84.y) = {1,1,1};
  float RimLitIntensity : packoffset(c85) = {4};
  float RimLitPower : packoffset(c85.y) = {2};
  float RimLightClampFactor : packoffset(c85.z) = {2};
  float ShadowReceiveOffset : packoffset(c85.w) = {0.600000024};
  float BloomIntensity : packoffset(c86) = {0.699999988};
  ...
  out float4 o2 : SV_TARGET2)
{
  ...
  r3.w = dot(LightDirForChar.xyz, r2.xyz);
  r3.xyz = r3.xyz * r1.www + LightDirForChar.xyz;
  r1.w = dot(r3.xyz, r3.xyz);
  r1.w = rsqrt(r1.w);
  ...
  r1.xy = w3.xy * float2(0.5,0.5) + float2(0.5,0.5);
  r1.xy = float2(15,15) * r1.xy;
  r1.xy = (uint2)r1.xy;
  r0.w = (uint)r1.x << 4;
  r0.w = (int)r1.y | (int)r0.w;
  r0.w = (uint)r0.w;
  o1.w = r0.w * 0.00392156886 + MaskEps;
  r0.w = v7.z / v7.w;
  r1.x = 256 * r0.w;
  r1.x = trunc(r1.x);
  r0.w = r0.w * 256 + -r1.x;
