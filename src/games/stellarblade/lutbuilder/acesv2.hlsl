...
static const float4x2 Chromaticities_AP0 = float4x2(
    0.73470, 0.26530,
    0.00000, 1.00000,
    0.00010, -0.07700,
    0.32168, 0.33767);
...
// Academy Primaries 1
static const float4x2 Chromaticities_AP1 = float4x2(
    0.713, 0.293,
    0.165, 0.830,
    0.128, 0.044,
    0.32168, 0.33767);
...
// Table generation
static const int tableSize = 360;
static const int additionalTableEntries = 2;  // allots for extra entries to wrap the hues
static const int totalTableSize = tableSize + additionalTableEntries;
static const int baseIndex = 1;  // array index for smallest filled entry of padded table

static const float hue_limit = 360.;

static const int cuspCornerCount = 6;
static const int totalCornerCount = cuspCornerCount + 2;
static const int max_sorted_corners = 2 * cuspCornerCount;
static const float reach_cusp_tolerance = 1e-3;
static const float display_cusp_tolerance = 1e-7;
...
static const float gamma_minimum = 0.0;
static const float gamma_maximum = 5.0;
static const float gamma_search_step = 0.4;
static const float gamma_accuracy = 1e-5;

// CAM Parameters
static const float ref_luminance = 100.;
static const float L_A = 100.;
static const float Y_b = 20.;
static const float3 surround = float3(0.9, 0.59, 0.9);  // Dim surround
...
static const float3x3 MATRIX_IDENTITY = float3x3(
    1, 0, 0,
    0, 1, 0,
    0, 0, 1);
...
HueTable build_hue_sample_interval(int samples,
  ...
  static const int totalTableSize = 362;
...
static const float testPositions[5] = { 0.01, 0.1, 0.5, 0.8, 0.99 };
