...
float4 main(
  ...
  if (!((bool)(_32.y == 200) && (bool)(abs((((((-0.0f - View_1040.y) - (View_1040.x * _34.x)) - (1.0f / ((View_1040.z * _34.x) - View_1040.w))) + View_1040.y) + (View_1040.x * _30.x)) + (1.0f / ((View_1040.z * _30.x) - View_1040.w))) < 0.009999999776482582f))) {
    ...
    float _147 = TEXCOORD_1.x * View_2168;

    float3 uv = float3(((saturate((log2(((_147 * (((cb0_061x + (cb0_066x * _115.x)) * _98.x) + (cb0_060x * _59.x))) * _146) + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f), ((saturate((log2(((_147 * (((cb0_061y + (cb0_066y * _115.y)) * _98.y) + (cb0_060y * _59.y))) * _146) + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f), ((saturate((log2(((_147 * (((cb0_061z + (cb0_066z * _115.z)) * _98.z) + (cb0_060z * _59.z))) * _146) + 0.002667719265446067f) * 0.0714285746216774f) + 0.6107269525527954f) * 0.96875f) + 0.015625f));

    float4 _175 = t6.Sample(s3, uv);
    // float3 _175 = renodx::lut::SampleTetrahedral(t6, uv, 32u);

    float _179 = _175.x * 1.0499999523162842f;
    float _180 = _175.y * 1.0499999523162842f;
    float _181 = _175.z * 1.0499999523162842f;
    float _184 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 0.00390625f) + -0.001953125f;
    float _185 = _179 + _184;
    float _186 = _180 + _184;
    float _187 = _181 + _184;
    float _188 = saturate(dot(float3(_179, _180, _181), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f)));
    [branch]
    if (!(cb0_065x == 0)) {
      float _199 = (pow(_185, 0.012683313339948654f));
      float _200 = (pow(_186, 0.012683313339948654f));
      float _201 = (pow(_187, 0.012683313339948654f));
      float _234 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_199 + -0.8359375f)) / (18.8515625f - (_199 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_064w));
      float _235 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_200 + -0.8359375f)) / (18.8515625f - (_200 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_064w));
      float _236 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_201 + -0.8359375f)) / (18.8515625f - (_201 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / cb0_064w));
      _262 = min((_234 * 12.920000076293945f), ((exp2(log2(max(_234, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
      ...
      _263 = _186;
      _264 = _187;
      _265 = saturate(renodx::color::y::from::BT709(renodx::color::srgb::DecodeSafe(float3(_185, _186, _187))));
    }
  } else {
    _262 = _59.x;
    _263 = _59.y;
